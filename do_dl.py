...
si, so, se = ssh.exec_command("cd /root/egg_video && base64 compressed_small.mp4", timeout=300)
encoded = "".join(so.readlines())
print(f"Got {len(encoded)} base64 chars")
...
print(f"Done! {sz} bytes ({sz/1024/1024:.2f}MB)")
ssh.close()
